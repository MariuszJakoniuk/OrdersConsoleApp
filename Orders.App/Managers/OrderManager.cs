...
public class OrderManager
{
    ...
    {
        Order? order = _orderService.Items.FirstOrDefault(x => x.Id == id);
        ShowOrdersTop();
        if (order != null)
        {
            ShowOdrerDetails(order);
    ...
    public int OrderStatusChange(int idChange)
    {
        ...
        if (toChange != 0)
        {
            ...
            //Order order = _orderService.GetItemById(idChange);
    ...
    public int EditOrder(int idEdit)
    {
        ...
        if (toEdit != 0)
        {
            ...
            if (Validation.GiveMeChar("Czy zmienić Id zamówienia? (T)ak/(N)ie: ", new char[] { 'T', 'N' }) == "T")
            {
                order.Id = Validation.GiveMeInt("Nowy Id zamówienia: ");
                count++;
            }
